...
question_1 = Quiz(
    question={
        "question": "What is the correct code snippet to calculate the weighting factors if SEX is the sensitive attribute?",
        "options": [
            "reweighing(df_transformed, model_target, 'SEX', True)",
            "reweighing(df, model_target, 'RAC1P', True)",
            "reweighing(df, model_target, 'SEX', True)",
            "None of the above",
        ],
        "correctIndex": 2,
    }
...
question_2 = Quiz(
    question={
        "question": "Did running the code give the same weights or different weights?",
        "options": ["Same weights", "Different weights", "Neither"],
        "correctIndex": 1,
    }
)
...
question_3 = Quiz(
    question={
        "question": "Which code snippet would adjust the outputs for the validation dataset?",
        "options": [
            "eo_model.predict(val_data[model_features], sensitive_features=val_data['SEX'].values)",
            "eo_model.predict(train_data[model_features], sensitive_features=train_data['RAC1P'].values)",
            "eo_model.predict(val_data[model_features], sensitive_features=val_data['RAC1P'].values)",
            "eo_model.predict(val_data)",
        ],
        "correctIndex": 2,
    }
)
